...
class BookmarkListViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func get_Posts(pageNo: NSInteger){      // to get bookmark list
        ...
        let newURL = "\(Constants.r2_baseURL)\("/get_bookmarks/")"
        print("\n new url :: \n",newURL)
        let headers = [
            ...
            if (error != nil) {
                ...
                do {
                    if let convertedJsonIntoDict = try JSONSerialization.jsonObject(with: data!, options: []) as? NSDictionary {
                        self.PostListDic = convertedJsonIntoDict.object(forKey: "get_bookmarks") as? NSDictionary
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if  dictObj["has_video"] as! String == "False" && dictObj["has_img"] as! String == "False" {
            ...
            textFeedCell.FeedLikeButton.tag = 1
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.performSegue(withIdentifier: "BookmarkListViewToViewMoreSegue", sender: self)
    }
    ...
    @objc func cellButtonTouchAction(_ sender: UIButton){
        DispatchQueue.main.async {
            if(sender.tag == 0) {
                ...
                //                if sender.currentImage == UIImage(named: "btn_like_unfilled"){
